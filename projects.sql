...
SELECT COUNT(tech_id) FROM tech LEFT OUTER JOIN project_uses_tech ON tech.id = project_uses_tech.tech_id GROUP BY tech.id;

SELECT * FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id;

SELECT count(project_id) FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id;

SELECT * FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id LEFT OUTER JOIN tech ON tech.id = project_uses_tech.tech_id;

SELECT DISTINCT(tech.name) FROM project INNER JOIN project_uses_tech ON project.id = project_uses_tech.project_id INNER JOIN tech ON tech.id = project_uses_tech.tech_id;

SELECT DISTINCT(tech.name) FROM project RIGHT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id RIGHT OUTER JOIN tech ON tech.id = project_uses_tech.tech_id WHERE project_id is NULL;
 
SELECT DISTINCT(project.name), count(project_uses_tech.tech_id) FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id GROUP BY project.id ORDER BY count;

SELECT DISTINCT(project.name), count(project_uses_tech.tech_id) FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id GROUP BY project.id ORDER BY count WHERE project_id is NULL;

SELECT tech.name, count(project_uses_tech.project_id) FROM tech LEFT OUTER JOIN project_uses_tech ON tech.id = project_uses_tech.tech_id ORDER BY count;

SELECT project.name, COUNT(project_uses_tech.tech_id) FROM project LEFT OUTER JOIN project_uses_tech ON project_uses_tech.project_id = project.id GROUP BY project.id ORDER BY count;

SELECT AVG(count) FROM (SELECT tech.name, count(project_uses_tech.project_id) FROM tech INNER JOIN project_uses_tech ON project_uses_tech.tech_id = tech.id GROUP BY tech.id) as count;
